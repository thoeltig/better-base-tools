...
server.registerTool(
  "batch_read",
  {
    ...
    description: "Batch-read N files in one call. Mode per file: 'compact'=DEFAULT (lossy: single-line for non-indent-sensitive files, collapses multi-ws, strips leading indent, minifies .json; use to understand content cheaply — verbatim/verbatim_numbered are the intended modes for edit anchors; replace supports fuzzy whitespace matching as a safety net if compact is accidentally used as anchor). 'verbatim'=byte-exact, no line numbers (use when formatting matters or as a readable anchor check). 'verbatim_numbered'=byte-exact + line-numbered (format: '{line}\\t{content}\\n'); use before insert_at_line / replace_range, or with searchTerm to get absolute line anchors for edits. 'fileinfo'=no content read; returns JSON {\"size\":N,\"lines\":N,\"mtimeMs\":N,\"ctimeMs\":N,\"isFile\":true}; use FIRST on unknown files to decide read strategy — check size+lines before committing tokens to a full read, then choose compact / verbatim / sliced offset+count accordingly. Path: accepts absolute file path, directory (expands to immediate children), or glob pattern (e.g. /proj/**/*.ts); all modes support glob/directory. Pagination: 'offset' (1-indexed start line) + 'count' (max lines) for read modes. Search: set 'searchTerm' for case-insensitive substring search; 'count' = context lines per match (default 0); result includes 'match_count' and blocks prefixed '<!-- Match at line N -->'. Search use-cases: (1) token-efficient multi-file scan — find all occurrences across a glob without reading full files; (2) safe global replace — search first to verify every occurrence matches intent, then use replace_all across N files with confidence; (3) with verbatim_numbered, match block line numbers are absolute file positions usable directly as replace_range / insert_at_line anchors.",
