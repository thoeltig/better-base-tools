...
server.registerTool(
  "batch_read",
  {
    ...
    description: "Batch-read N files in one call. Mode per file: 'compact'=DEFAULT — lowest token cost; strips indent/whitespace; use for full-file reads and as replace/replace_all anchor. 'verbatim'= exact content — full-file replace anchor; sliced reads (offset+count) include line range in output header for replace_range/insert_at_line anchoring. 'fileinfo'= metadata (size, lines, ISO mtime, isFile) plus refs[] of resolved file references. Mode→op pairing: compact → replace/replace_all | verbatim → replace/replace_all | verbatim+offset+count → replace_range/insert_at_line (use line range from output header). Path: absolute or relative file, directory (expands to immediate children) or glob (e.g. proj/**/*.ts); all modes support glob/directory. Pagination: 'offset' (1-indexed start line) + 'count' (max lines). Search: 'searchTerm' (case-insensitive) + 'count' (context lines around each match); omit count for per-line results with line numbers, set count for context windows with file-position anchors. Use cases — follow this cascade for unknown files: (1) fileinfo — check size+lines before reading content, map deps via refs[]; (2) compact — full-file overview or replace/replace_all anchor (lowest token cost); (3) verbatim — full-file replace anchor when compact whitespace stripping would break the match; (4) verbatim+offset+count — targeted slice; use line range from output header to anchor replace_range/insert_at_line. Other use cases: (5) multi-file scan — searchTerm+glob finds occurrences across files without full reads; (6) dependency map — fileinfo+glob returns metadata+refs[] per file; (7) safe global replace — compact+searchTerm to verify occurrences, then replace_all;",
