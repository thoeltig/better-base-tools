...
server.registerTool(
  "batch_read",
  {
    ...
    description: "Batch-read N files in one call. Mode per file: 'compact'=DEFAULT (single-line collapsed, minified file content). 'verbatim'= file content as is, no line numbers. 'verbatim_numbered'= file content as is with line-numbers (format: '{line}\\t{content}'). 'fileinfo'= file metadata (size, lines, ISO mtime, isFile) with referenced related files. Path: absolute file, directory (expands to immediate children) or glob (e.g. /proj/**/*.ts); all modes support glob/directory. Pagination: 'offset' (1-indexed start line) + 'count' (max lines). Search: 'searchTerm' (case-insensitive match) + 'count' (lines around the match, default 0). Use cases: (1) unknown file — fileinfo first, check size+lines, then choose mode + offset+count; (1b) dependency map — fileinfo on a file or glob returns metadata + all related files per file; (2) multi-file scan — searchTerm + glob finds occurrences without full reads; (3) edit prep — 'verbatim_numbered' + searchTerm gives absolute line anchors for 'replace_range' or 'insert_at_line' edits; (4) safe global replace — 'verbatim' + searchTerm gives anchors for 'replace_all' edits. (5) 'compact' for minified content to gather information (6) 'verbatim' to get anchor for old / new  'replace' edit (7) set 'disableNormalizedFormatting' to true to receive the formatted file content in case user asks for file file formatting, not needed for normal read/edit workflow. Note: 'compact' is for information gathering rather than edit anchors. In case you do use 'compact' content as anchors for editing, whitespace mismatches are handled by the 'batch_edit' tool's fuzzy matching but the intended use cases are 'verbatim' mode for old/new replacements or 'verbatim' for insert or range replace.",
