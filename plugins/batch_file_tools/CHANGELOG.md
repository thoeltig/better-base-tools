...
- **Op errors are identified by content instead of by array index** — `<!-- op 1 (replace); error: ... -->` became `<!-- op (replace) old: "MISSING"; error: ... -->`. The leading index read like a count and collided with the `N/M ops successful` figure on the line above; the op type plus a 40-char excerpt of what the op addressed (anchor, line range, or write mode) says which op failed without the reader having to map indices. Skipped ops are listed the same way, one line each (`<!-- op (write) mode: append; skipped -->`): line-addressed ops execute before content-addressed ones, so execution order differs from request order and neither an index nor a count would tell the model which ops landed. `OpResult` gains an optional `target` carrying the excerpt, now attached to every non-ok op rather than errors alone.
- **Whitespace-fallback test names and comments reframed** — they described themselves as composing with normalized `verbatim` reads, which no longer exist. Each now names the invariant it protects: whitespace-only gaps are bridged, differences beyond whitespace are refused, and an edit never reformats the line it touches. The anchors these tests exercise come from the model writing indentation from habit, not from tool output.
...
- **Tests were never type-checked, and several passed only by accident** — `tests/` was excluded from `tsc`, so `npm run typecheck` now uses a `tsconfig.typecheck.json` covering `src/` and `tests/` (the build still emits from `src` alone). Fixed the 11 errors it surfaced: ten `handleBatchEdit` calls omitted the required `dryRun` argument and only wrote to disk because the fallback value is falsy, while passing a dead `dryRun` key left over from when it was a request field; plus a `verbose` key no schema defines, the removed `verbatim_numbered` mode, and `ops: object[]`.
- **`stopOnError` discarded ops that had already succeeded** — an aborting op set `abortedOps`, which suppressed the file write entirely, so ops applied before the failure never reached disk while still being counted in `N/M ops successful`. A 5-op file failing at op 4 reported `3/5 ops successful` against an unchanged file. The write no longer consults `abortedOps`: earlier ops persist, the failing op reports `error`, later ops report `skipped`, and the count matches what landed. The behaviour was undocumented and had no test — every existing `stopOnError` case put the failing op first, so nothing had ever succeeded before the abort.
