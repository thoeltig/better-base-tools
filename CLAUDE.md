...
|---|---|
| `Read` | `batch_read` (mode=`edit` for pre-edit anchor reads, `raw` for verbatim, `compact` for scanning) |
| `Edit` | `batch_edit` with appropriate op (`replace`, `replace_all`, `insert_at_line`, `replace_range`, `append`, `delete`, `create`, `overwrite`) |
| `Write` | `batch_edit` with `create` (new file) or `overwrite` (replace existing) op |
